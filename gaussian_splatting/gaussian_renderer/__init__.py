...
def generate_neural_gaussians(viewpoint_camera, pc : GaussianModel, visible_mask=None, is_training=False):
    ...
    # ================= START: 防止空点云导致崩溃的保护补丁 =================
    # if anchor.shape[0] == 0:
    #     dev = anchor.device
    #     # 构建空的输出，让渲染器直接画背景色，安全跳过当前帧
    #     empty_xyz = torch.zeros((0, 3), device=dev)
    #     empty_color = torch.zeros((0, 3), device=dev)
    #     empty_opacity = torch.zeros((0, 1), device=dev)
    #     empty_scaling = torch.zeros((0, 3), device=dev)
    #     empty_rot = torch.zeros((0, 4), device=dev)
        
    #     if is_training:
    #         empty_neural_opacity = torch.zeros((0, 1), device=dev)
    #         empty_mask = torch.zeros((0,), dtype=torch.bool, device=dev)
    #         return empty_xyz, empty_color, empty_opacity, empty_scaling, empty_rot, empty_neural_opacity, empty_mask
    #     else:
    #         return empty_xyz, empty_color, empty_opacity, empty_scaling, empty_rot
    # ================= END: 防止空点云导致崩溃的保护补丁 =================
