...
def evaluate_trj(trj_path, alignment):
    ...
    scene = trj_path.parents[2].name.replace("GigaSLAM_railway_data_", "")
...
def save_grid_plot(items, output_dir, cmap, norm, limits, args):
    fig, axes = plt.subplots(2, 3, figsize=(18, 9.8), dpi=args.dpi)
    axes = axes.ravel()
...
def write_manifest(output_dir, items, limits, args, colorbar_vmax, grid_png, grid_pdf):
    manifest = {
...
def main():
    ...
    trj_paths = sorted(args.results_dir.glob(f"GigaSLAM_railway_data_scene_*_train/*-No-LC/plot/trj_{args.label}.json"))
    if not trj_paths:
        raise FileNotFoundError(f"No trj_{args.label}.json files found under {args.results_dir}")
    ...
    grid_png, grid_pdf = save_grid_plot(items, args.output_dir, cmap, norm, limits, args)
    manifest_path = write_manifest(args.output_dir, items, limits, args, vmax, grid_png, grid_pdf)
