...
class SLAM_GUI:
    def __init__(self, params_gui=None):
        ...
        if params_gui is not None:
            ...
            self.gaussian_model = params_gui.gaussians  # keep model with MLPs
            self.init = True
    ...
    def receive_data(self, q):
        ...
        if gaussian_packet.has_gaussians:
            ...
            if hasattr(gaussian_packet, '_anchor') and hasattr(self, 'gaussian_model'):
                dev = self.gaussian_model._anchor.device
                self.gaussian_model._anchor = gaussian_packet._anchor.to(dev)
                self.gaussian_model._anchor_feat = gaussian_packet._anchor_feat.to(dev)
                self.gaussian_model._offset = gaussian_packet._offset.to(dev)
                self.gaussian_model._scaling = gaussian_packet._scaling.to(dev)
                self.gaussian_model._opacity = gaussian_packet._opacity.to(dev)
                self.gaussian_model._rotation = gaussian_packet._rotation.to(dev)
                self.gaussian_model._level = gaussian_packet._level.to(dev)
            anchor_count = self.gaussian_cur.get_anchor.shape[0] if hasattr(self.gaussian_cur, 'get_anchor') else self.gaussian_cur.get_xyz.shape[0]
            self.output_info.text = "Number of Gaussians: {}".format(anchor_count)
            self.init = True
        ...
        if gaussian_packet.gtcolor is not None:
            rgb = gaussian_packet.gtcolor
            if isinstance(rgb, torch.Tensor):
                rgb = (torch.clamp(rgb, 0, 1.0) * 255).byte().permute(1, 2, 0).contiguous().cpu().numpy()
            try:
                rgb = o3d.geometry.Image(np.ascontiguousarray(rgb))
                self.in_rgb_widget.update_image(rgb)
            except Exception as e:
                Log(f"gtcolor display error: {e} shape={np.asarray(gaussian_packet.gtcolor).shape}", tag="GUI")

        if gaussian_packet.gtdepth is not None:
            depth = gaussian_packet.gtdepth
            depth = imgviz.depth2rgb(
                depth, min_value=0.1, max_value=float(np.max(depth)) if np.max(depth) > 0 else 255.0, colormap="jet"
            )
    ...
    def rasterise(self, current_cam):
        if self.gaussian_cur is None or self.gaussian_cur.get_anchor.shape[0] == 0:
            return None
        # Use the GUI-local model (has MLPs) with updated tensors from GaussianPacket
        pc = self.gaussian_model if hasattr(self, 'gaussian_model') else self.gaussian_cur
        if (
            self.time_shader_chbox.checked
            and self.gaussian_cur is not None
            and type(self.gaussian_cur) == GaussianPacket
        ):
            features = pc.get_features.clone()
            ...
            alpha = 0.1
            pc.get_features = alpha * features + (
                1 - alpha
